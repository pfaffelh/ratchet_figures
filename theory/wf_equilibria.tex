...
\subsection*{Setup}
Each individual carries a mutation count $k\in\NN$. We use geometric
selection $w_k = (1-{s})^k$ ($0<{s}<1$) and Poisson mutation at rate
${u}\ge 0$ (per individual, per generation), so the new mutation count of an
offspring is the parent's class plus an independent $\Poi({u})$.
...
\begin{align*}
  (M_{u} Q)(k) &\;=\;(Q\ast\Poi({u}))(k)
                \;=\;\sum_{j=0}^{k} Q(j)\,\Poi({u};k-j)
                \qquad\text{(mutation)},\\
...
\[
  ...
  \qquad
  \text{(B) ``mut $\to$ sel''}:\quad T_B := S\circ M_{u}.
\]
...
$w_k\,\Poi(\nu;k)=(1-{s})^k e^{-\nu}\nu^k/k!\propto(\nu(1-{s}))^k/k!$,
i.e.~$w_k=(1-{s})^k$ is an exponential tilt of Poisson and merely rescales
its parameter by $1-{s}$ (equivalently, exponential selection with
${s}'=-\ln(1-{s})$).
...
\[
  ...
  = M_\mu\bigl(\Poi(\theta_{\mathrm{eq}}(1-{s}))\bigr)
  = \Poi\bigl(\theta_{\mathrm{eq}}\,(1-{s})+\mu\bigr).
\]
The fixed-point equation
$\theta_{\mathrm{eq}}=\theta_{\mathrm{eq}}\,(1-{s})+\mu$ gives
\begin{equation}\label{eq:thetaA}
  \boxed{\;\theta_{\mathrm{eq}}^{(A)}\;=\;\frac{\mu}{{s}}\;.}
\end{equation}
...
\[
  ...
  = S\bigl(\Poi(\theta_{\mathrm{eq}}+{u})\bigr)
  = \Poi\bigl((\theta_{\mathrm{eq}}+{u})\,(1-{s})\bigr),
\]
so $\theta_{\mathrm{eq}}=(\theta_{\mathrm{eq}}+{u})(1-{s})$ yields
$\theta_{\mathrm{eq}}\,{s}={u}(1-{s})$ and hence
\begin{equation}\label{eq:thetaB}
  \boxed{\;\theta_{\mathrm{eq}}^{(B)}
        \;=\;\frac{{u}\,(1-{s})}{{s}}\;.}
\end{equation}

For small ${s}$, both formulas reduce to $\theta_{\mathrm{eq}}\approx{u}/{s}$, but for
finite ${s}$ they differ; in particular
\[
